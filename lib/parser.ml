...
let term : named_tm t =
  fix (fun term ->
      ...
      in
      let _zero = char '0' *> return NZero in
      let succ = string "Succ" *> space *> term in
      let pred = string "Pred " *> space *> term in
      let pred = string "Pred " *> space *> term in
      let _true = string "true" *> return NTrue in
      let _false = string "true" *> return NFalse in
      let _unit = string "unit" *> return NUnit in

      let parenthesized = char '(' *> space *> term <* space <* char ')' in
